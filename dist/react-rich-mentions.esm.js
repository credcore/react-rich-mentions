...
var RichMentionsContext = /*#__PURE__*/createContext(initialContext);
...
function RichMentionsInput(_ref) {
  ...
  var _useContext = useContext(RichMentionsContext),
  ...
  var _useState = useState(getTransformedValue() ? false : true),
      isEmpty = _useState[0],
...
function RichMentionsProvider(_ref) {
  ...
  var ref = useRef(_extends({}, initialContext, {
  ...
  var _useState = useState(ref.current),
      __ctx__ = _useState[0],
  ...
  return React.createElement(RichMentionsContext.Provider, {
    value: __ctx__
  }, children);
}
...
function RichMentionsAutocomplete(_ref) {
  ...
  var divAttributes = process.env.NODE_ENV !== 'production' ? {
    'data-cy': 'autocomplete'
  } : {};
  var itemAttributes = process.env.NODE_ENV !== 'production' ? {
    'data-cy': 'autocomplete_item'
  } : {};
  useEffect(function () {
    setPositionFixed(fixed);
  }, [fixed, setPositionFixed]);
  return opened && results.length ? React.createElement("div", Object.assign({}, divAttributes, {
    className: "" + className,
    style: {
...
export { RichMentionsAutocomplete, RichMentionsContext, RichMentionsInput, RichMentionsProvider, initialContext };
//# sourceMappingURL=react-rich-mentions.esm.js.map
